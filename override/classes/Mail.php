...
class Mail extends MailCore
{
        ...
        $template_path = _PS_MAIL_DIR_, $die = false, $id_shop = null, $bcc = null, $reply_to = null)
    {
        ...
        $message = SwiftUpgrade_Message::newInstance();
        ...
        try {
            ...
            if ($configuration['PS_MAIL_METHOD'] == 2) {
                ...
                $connection = SwiftUpgrade_SmtpTransport::newInstance($configuration['PS_MAIL_SERVER'], $configuration['PS_MAIL_SMTP_PORT'], $configuration['PS_MAIL_SMTP_ENCRYPTION'])
                    ->setUsername($configuration['PS_MAIL_USER'])
                    ->setPassword($configuration['PS_MAIL_PASSWD']);

            } else {
                $connection = SwiftUpgrade_MailTransport::newInstance();
            }

            if (!$connection) {
                return false;
            }
            $swift = SwiftUpgrade_Mailer::newInstance($connection);
            ...
            $swift->registerPlugin(new SwiftUpgrade_Plugins_DecoratorPlugin(array($to_plugin => $template_vars)));
            ...
            if ($file_attachment && !empty($file_attachment)) {
                ...
                foreach ($file_attachment as $attachment) {
                    if (isset($attachment['content']) && isset($attachment['name']) && isset($attachment['mime'])) {
                        $message->attach(SwiftUpgrade_Attachment::newInstance()->setFilename($attachment['name'])->setContentType($attachment['mime'])->setBody($attachment['content']));
                    }
                }
            }
            /* Send mail */
            ...
            if ($send && Configuration::get('PS_LOG_EMAILS')) {
                ...
                foreach (array_merge($recipients_to, $recipients_cc, $recipients_bcc) as $email => $recipient_name) {
                    /** @var SwiftUpgrade_Address $recipient */
                    $mail->id = null;
            ...
            return $send;
        } catch (SwiftUpgrade_SwiftException $e) {
            PrestaShopLogger::addLog(
                'Swift Error: '.$e->getMessage(),
                3,
                null,
                'SwiftUpgrade_Message'
            );
    ...
    public static function sendMailTest($smtp_checked, $smtp_server, $content, $subject, $type, $to, $from, $smtp_login, $smtp_password, $smtp_port = 25, $smtp_encryption)
    {
        ...
        try {
            if ($smtp_checked) {
                ...
                $smtp = SwiftUpgrade_SmtpTransport::newInstance($smtp_server, $smtp_port, $smtp_encryption)
                    ->setUsername($smtp_login)
                    ->setPassword($smtp_password);
                $swift = SwiftUpgrade_Mailer::newInstance($smtp);
            } else {
                $swift = SwiftUpgrade_Mailer::newInstance(SwiftUpgrade_MailTransport::newInstance());
            }
    ...
    {
        include_once(_PS_MODULE_DIR_.'swiftupgrader/lib/swiftlatest/swift_required.php');
    }
}

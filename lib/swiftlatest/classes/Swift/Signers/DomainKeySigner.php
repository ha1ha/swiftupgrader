 ...
 */
class SwiftUpgrade_Signers_DomainKeySigner implements SwiftUpgrade_Signers_HeaderSigner
{
    /**
     * PrivateKey.
     *
     ...
     *
     * @var SwiftUpgrade_Mime_Headers_ParameterizedHeader
     */
     ...
     * @param string $selector
     *
     * @return SwiftUpgrade_Signers_DomainKeySigner
     */
            ...
            switch ($string[$i]) {
                ...
                case "\n" :
                    if ($this->_bodyCanonLastChar == "\r") {
                        ...
                        // Wooops Error
                        throw new SwiftUpgrade_SwiftException('Invalid new line sequence in mail found \n without preceding \r');
                    }
                    break;
                case ' ' :
                case "\t" :
    ...
    /**
     * @throws SwiftUpgrade_SwiftException
     *
     * @return string
     */
    private function _getEncryptedHash()
    {
        ...
        if (!$pkeyId) {
            throw new SwiftUpgrade_SwiftException('Unable to load DomainKey Private Key ['.openssl_error_string().']');
        }
        if (openssl_sign($this->_canonData, $signature, $pkeyId, OPENSSL_ALGO_SHA1)) {
            return $signature;
        }
        throw new SwiftUpgrade_SwiftException('Unable to sign DomainKey Hash  ['.openssl_error_string().']');
    }
}

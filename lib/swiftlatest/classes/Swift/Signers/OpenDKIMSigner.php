 ...
 */
class SwiftUpgrade_Signers_OpenDKIMSigner extends SwiftUpgrade_Signers_DKIMSigner
{
    ...
    public function __construct($privateKey, $domainName, $selector)
    {
        if (!extension_loaded('opendkim')) {
            throw new SwiftUpgrade_SwiftException('php-opendkim extension not found');
        }
    ...
    {
        $header = new SwiftUpgrade_Mime_Headers_OpenDKIMHeader('DKIM-Signature');
        $headerVal = $this->_dkimHandler->getSignatureHeader();
        if (!$headerVal) {
            throw new SwiftUpgrade_SwiftException('OpenDKIM Error: '.$this->_dkimHandler->getError());
        }
    ...
    public function setHeaders(SwiftUpgrade_Mime_HeaderSet $headers)
    {
        ...
        if (!is_numeric($this->_signatureTimestamp)) {
            ...
            if (!OpenDKIM::setOption(OpenDKIM::OPTS_FIXEDTIME, $this->_signatureTimestamp)) {
                throw new SwiftUpgrade_SwiftException('Unable to force signature timestamp ['.openssl_error_string().']');
            }
        }

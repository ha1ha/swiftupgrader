 ...
 */
class SwiftUpgrade_Signers_DKIMSigner implements SwiftUpgrade_Signers_HeaderSigner
{
    /**
     * PrivateKey.
     *
     ...
     * @param string $selector
     *
     * @return SwiftUpgrade_Signers_DKIMSigner
     */
    ...
    private function _getEncryptedHash()
    {
        ...
        if (!$pkeyId) {
            throw new SwiftUpgrade_SwiftException('Unable to load DKIM Private Key ['.openssl_error_string().']');
        }
        if (openssl_sign($this->_headerCanonData, $signature, $pkeyId, $algorithm)) {
            return $signature;
        }
        throw new SwiftUpgrade_SwiftException('Unable to sign DKIM Hash ['.openssl_error_string().']');
    }
}

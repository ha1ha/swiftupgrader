 ...
 */
class SwiftUpgrade_Signers_SMimeSigner implements SwiftUpgrade_Signers_BodySigner
{
    ...
    /**
     * @var SwiftUpgrade_StreamFilters_StringReplacementFilterFactory
     */
    protected $replacementFactory;

    /**
     * @var SwiftUpgrade_Mime_HeaderFactory
     */
    ...
    public function __construct($signCertificate = null, $signPrivateKey = null, $encryptCertificate = null)
    {
        ...
        $this->replacementFactory = SwiftUpgrade_DependencyContainer::getInstance()
    ...
    /**
     * Returns an new SwiftUpgrade_Signers_SMimeSigner instance.
     *
     * @param string $certificate
     * @param string $privateKey
     *
     * @return SwiftUpgrade_Signers_SMimeSigner
     */
    ...
    public function signMessage(SwiftUpgrade_Message $message)
    {
        ...
        $messageStream = new SwiftUpgrade_ByteStream_TemporaryFileByteStream();
        $this->toSMimeByteStream($messageStream, $message);
        $message->setEncoder(SwiftUpgrade_DependencyContainer::getInstance()->lookup('mime.rawcontentencoder'));
    ...
    /**
     * @param SwiftUpgrade_InputByteStream $inputStream
     * @param Swift_Message         $mimeEntity
     */
    protected function toSMimeByteStream(SwiftUpgrade_InputByteStream $inputStream, SwiftUpgrade_Message $message)
    {
        ...
        if (null !== $this->signCertificate && null !== $this->encryptCert) {
            $temporaryStream = new SwiftUpgrade_ByteStream_TemporaryFileByteStream();
     ...
     */
    protected function createMessage(SwiftUpgrade_Message $message)
    {
        $mimeEntity = new SwiftUpgrade_Message('', $message->getBody(), $message->getContentType(), $message->getCharset());
    ...
    /**
     * @param SwiftUpgrade_FileStream      $outputStream
     * @param SwiftUpgrade_InputByteStream $inputStream
     *
     * @throws SwiftUpgrade_IoException
     */
    protected function messageStreamToSignedByteStream(SwiftUpgrade_FileStream $outputStream, SwiftUpgrade_InputByteStream $inputStream)
    {
        $signedMessageStream = new SwiftUpgrade_ByteStream_TemporaryFileByteStream();
        ...
        if (!call_user_func_array('openssl_pkcs7_sign', $args)) {
            throw new SwiftUpgrade_IoException(sprintf('Failed to sign S/Mime message. Error: "%s".', openssl_error_string()));
        }
     ...
     * @throws SwiftUpgrade_IoException
     */
    protected function messageStreamToEncryptedByteStream(SwiftUpgrade_FileStream $outputStream, SwiftUpgrade_InputByteStream $is)
    {
        $encryptedMessageStream = new SwiftUpgrade_ByteStream_TemporaryFileByteStream();

        if (!openssl_pkcs7_encrypt($outputStream->getPath(), $encryptedMessageStream->getPath(), $this->encryptCert, array(), 0, $this->encryptCipher)) {
            throw new SwiftUpgrade_IoException(sprintf('Failed to encrypt S/Mime message. Error: "%s".', openssl_error_string()));
        }
    ...
    /**
     * @param SwiftUpgrade_OutputByteStream $fromStream
     * @param SwiftUpgrade_InputByteStream  $toStream
     */
    protected function copyFromOpenSSLOutput(SwiftUpgrade_OutputByteStream $fromStream, SwiftUpgrade_InputByteStream $toStream)
    {
        $bufferLength = 4096;
        $filteredStream = new SwiftUpgrade_ByteStream_TemporaryFileByteStream();
     ...
     * @param Swift_Message          $message
     */
    protected function streamToMime(SwiftUpgrade_OutputByteStream $fromStream, SwiftUpgrade_Message $message)
    {
        $bufferLength = 78;
        $headerData = '';
        ...
        if ('multipart/signed;' === substr($headers['content-type'], 0, 17)) {
            if (!preg_match('/boundary=("[^"]+"|(?:[^\s]+|$))/is', $headers['content-type'], $contentTypeData)) {
                throw new SwiftUpgrade_SwiftException('Failed to find Boundary parameter');
            }
            ...
            if (null === $this->headerFactory) {
                $this->headerFactory = SwiftUpgrade_DependencyContainer::getInstance()->lookup('mime.headerfactory');
            }

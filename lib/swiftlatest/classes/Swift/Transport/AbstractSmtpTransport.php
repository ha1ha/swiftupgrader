 ...
 * @author Chris Corbyn
 */
abstract class SwiftUpgrade_Transport_AbstractSmtpTransport implements SwiftUpgrade_Transport
{
     ...
     *
     * @param SwiftUpgrade_Transport_IoBuffer     $buf
     * @param Swift_Events_EventDispatcher $dispatcher
     */
    public function __construct(SwiftUpgrade_Transport_IoBuffer $buf, SwiftUpgrade_Events_EventDispatcher $dispatcher)
    {
     ...
     * @param string $domain
     *
     * @return SwiftUpgrade_Transport_AbstractSmtpTransport
     */
    ...
    public function send(SwiftUpgrade_Mime_Message $message, &$failedRecipients = null)
    {
        ...
        if (!$reversePath = $this->_getReversePath($message)) {
            $this->_throwException(new SwiftUpgrade_TransportException(
        ...
        if ($evt) {
            if ($sent == count($to) + count($cc) + count($bcc)) {
                $evt->setResult(SwiftUpgrade_Events_SendEvent::RESULT_SUCCESS);
            } elseif ($sent > 0) {
                $evt->setResult(SwiftUpgrade_Events_SendEvent::RESULT_TENTATIVE);
            } else {
                $evt->setResult(SwiftUpgrade_Events_SendEvent::RESULT_FAILED);
            }
     ...
     *
     * @return SwiftUpgrade_Transport_IoBuffer
     */
    ...
    protected function _streamMessage(SwiftUpgrade_Mime_Message $message)
    {
    ...
    protected function _getReversePath(SwiftUpgrade_Mime_Message $message)
    {
    ...
    protected function _throwException(SwiftUpgrade_TransportException $e)
    {
    ...
    protected function _assertResponseCode($response, $wanted)
    {
        ...
        if (!$valid) {
            $this->_throwException(
                new SwiftUpgrade_TransportException(
    ...
    protected function _getFullResponse($seq)
    {
        ...
        try {
            ...
            $this->_throwException($e);
        } catch (SwiftUpgrade_IoException $e) {
            $this->_throwException(
                new SwiftUpgrade_TransportException(
                    $e->getMessage())
                );
    ...
    private function _sendTo(SwiftUpgrade_Mime_Message $message, $reversePath, array $to, array &$failedRecipients)
    {
        if (empty($to)) {
            return 0;
        }
    ...
    private function _sendBcc(SwiftUpgrade_Mime_Message $message, $reversePath, array $bcc, array &$failedRecipients)
    {
        $sent = 0;

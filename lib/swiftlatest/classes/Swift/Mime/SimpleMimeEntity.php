 ...
 * @author Chris Corbyn
 */
class SwiftUpgrade_Mime_SimpleMimeEntity implements SwiftUpgrade_Mime_MimeEntity
{
     ...
     */
    public function __construct(SwiftUpgrade_Mime_HeaderSet $headers, SwiftUpgrade_Mime_ContentEncoder $encoder, SwiftUpgrade_KeyCache $cache, SwiftUpgrade_Mime_Grammar $grammar)
    {
    ...
    /**
     * Get the {@link SwiftUpgrade_Mime_HeaderSet} for this entity.
     *
     * @return SwiftUpgrade_Mime_HeaderSet
     */
     ...
     *
     * @return SwiftUpgrade_Mime_MimeEntity[]
     */
     ...
     *
     * @param SwiftUpgrade_Mime_MimeEntity[] $children
    ...
    {
        return $this->_body instanceof SwiftUpgrade_OutputByteStream ? $this->_readStream($this->_body) : $this->_body;
    }

    /**
     * Set the body of this entity, either as a string, or as an instance of
     * {@link SwiftUpgrade_OutputByteStream}.
     *
     * @param mixed  $body
     ...
     *
     * @return SwiftUpgrade_Mime_ContentEncoder
     */
     ...
     * @return Swift_Mime_SimpleMimeEntity
     */
    public function setEncoder(SwiftUpgrade_Mime_ContentEncoder $encoder)
    {
     ...
     *
     * @throws SwiftUpgrade_RfcComplianceException
     *
     * @return Swift_Mime_SimpleMimeEntity
     */
     ...
     * @param SwiftUpgrade_Mime_ContentEncoder $encoder
     */
    public function encoderChanged(SwiftUpgrade_Mime_ContentEncoder $encoder)
    {
        $this->_notifyEncoderChanged($encoder);
    }
    ...
    protected function _bodyToString()
    {
        ...
        if (isset($this->_body) && empty($this->_immediateChildren)) {
            if ($this->_cache->hasKey($this->_cacheKey, 'body')) {
                ...
                $this->_cache->setString($this->_cacheKey, 'body', $body, SwiftUpgrade_KeyCache::MODE_WRITE);
            }
            $string .= $body;
        }
    ...
    /**
     * Write this entire entity to a {@see SwiftUpgrade_InputByteStream}.
     *
     * @param SwiftUpgrade_InputByteStream
     */
    public function toByteStream(SwiftUpgrade_InputByteStream $is)
    {
    ...
    /**
     * Write this entire entity to a {@link SwiftUpgrade_InputByteStream}.
     *
     * @param SwiftUpgrade_InputByteStream
     */
    protected function _bodyToByteStream(SwiftUpgrade_InputByteStream $is)
    {
        if (empty($this->_immediateChildren)) {
            if (isset($this->_body)) {
                if ($this->_cache->hasKey($this->_cacheKey, 'body')) {
                    ...
                    if ($this->_body instanceof SwiftUpgrade_OutputByteStream) {
    ...
    protected function getRandomId()
    {
        ...
        } catch (SwiftUpgrade_RfcComplianceException $e) {
    ...
    private function _readStream(SwiftUpgrade_OutputByteStream $os)
    {
        $string = '';
    ...
    private function _assertValidBoundary($boundary)
    {
        if (!preg_match('/^[a-z0-9\'\(\)\+_\-,\.\/:=\?\ ]{0,69}[a-z0-9\'\(\)\+_\-,\.\/:=\?]$/Di', $boundary)) {
            throw new SwiftUpgrade_RfcComplianceException('Mime boundary set is not RFC 2046 compliant.');
        }
    }
    ...
    private function _notifyEncoderChanged(SwiftUpgrade_Mime_ContentEncoder $encoder)
    {
        foreach ($this->_immediateChildren as $child) {
    ...
    private function _assertValidId($id)
    {
        if (!preg_match('/^'.$this->_grammar->getDefinition('id-left').'@'.$this->_grammar->getDefinition('id-right').'$/D', $id)) {
            throw new SwiftUpgrade_RfcComplianceException('Invalid ID given <'.$id.'>');
        }
    }

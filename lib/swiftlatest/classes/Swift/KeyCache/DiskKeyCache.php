 ...
 * @author Chris Corbyn
 */
class SwiftUpgrade_KeyCache_DiskKeyCache implements SwiftUpgrade_KeyCache
{
     ...
     *
     * @var SwiftUpgrade_KeyCache_KeyCacheInputStream
     */
    private $_stream;
     ...
     *
     * @param SwiftUpgrade_KeyCache_KeyCacheInputStream $stream
     * @param string                             $path   to save to
     */
    public function __construct(SwiftUpgrade_KeyCache_KeyCacheInputStream $stream, $path)
    {
        $this->_stream = $stream;
        $this->_path = $path;
    ...
    public function setString($nsKey, $itemKey, $string, $mode)
    {
        ...
        switch ($mode) {
            ...
            case self::MODE_APPEND:
                ...
                break;
            default:
                throw new SwiftUpgrade_SwiftException(
     ...
     * @throws SwiftUpgrade_IoException
     */
    public function importFromByteStream($nsKey, $itemKey, SwiftUpgrade_OutputByteStream $os, $mode)
    {
        $this->_prepareCache($nsKey);
        switch ($mode) {
            case self::MODE_WRITE:
     ...
     * @param string                $nsKey
     * @param string                $itemKey
     * @param SwiftUpgrade_InputByteStream $writeThrough
     *
     * @return SwiftUpgrade_InputByteStream
     */
    public function getInputByteStream($nsKey, $itemKey, SwiftUpgrade_InputByteStream $writeThrough = null)
    {
     ...
     * @param string                $nsKey
     * @param string                $itemKey
     * @param SwiftUpgrade_InputByteStream $is      to write the data to
     */
    public function exportToByteStream($nsKey, $itemKey, SwiftUpgrade_InputByteStream $is)
    {
        if ($this->hasKey($nsKey, $itemKey)) {
            $fp = $this->_getHandle($nsKey, $itemKey, self::POSITION_START);
            if ($this->_quotes) {
    ...
    private function _prepareCache($nsKey)
    {
        ...
        if (!is_dir($cacheDir)) {
            if (!mkdir($cacheDir)) {
                throw new SwiftUpgrade_IoException('Failed to create cache directory '.$cacheDir);
            }
